...
var samples = samples.filter(ee.Filter.neq('Label',7))

var samples = samples.limit(200)
...
var s2 =  getS2('2021-01-01', '2021-12-31')
...
var layer = s2.addBands(SAR20_HH).addBands(SAR20_HV).addBands(SAR1520_HH).addBands(SAR1520_HV).addBands(GLO30).addBands(DSM05)
            ...
            .addBands(label).addBands(S1_VH).addBands(S1_VV).addBands(hand30).addBands(DTM05_SM5).addBands(Slope_SM5)
...
var bands_all = samples.first().propertyNames().removeAll(["system:index","label", "dif_GLO30","Slope_SM5","Slope_SM10","DTM05_SM10","DTM05_SM3","Slope_SM3"])                                      
var df_bands_all = samples.select(bands_all)

var rf = ee.Classifier.smileRandomForest({numberOfTrees: 300, minLeafPopulation:10}).setOutputMode('REGRESSION')
...
var observation_prediction = height_layer.sampleRegions({collection:df_bands_all, properties: bands_all, scale:10, tileScale:10})
...
print(observation_prediction.first())

// Export the FeatureCollection.
Export.table.toDrive({
  collection: observation_prediction,
  description: 'Segura_all_variables',
  fileFormat: 'CSV'
});
...
/// Selected bands 2
var df_bands_all = samples.select(bands_all)
var bands_selection = ["rh98", "SAR1520_HV", "SAR20_HV", "S1VH", "B12_p95", "SAR1520_HH", "B8_p20", "B12_p90", "ndmi_p10", "B12_p50", "dif_DM05", 
                       "ndmi_p20", "SAR20_HH", "B11_p20" , "DTM05" , "DTM05_SM5" , "GLO30", "DSM05","B11_p90","B11_p95", "B2_p10"]
var df_bands_all = df_bands_all.select(bands_selection)
...
Export.table.toDrive({
  collection: observation_prediction,
  description: 'Segura_all_variables_20',
  fileFormat: 'CSV'
});

/// Selected bands 3
var df_bands_all = samples.select(bands_all)
var bands_selection = ["rh98" , "Slope_SM5" , "SAR1520_HV" , "SAR20_HV" , "SAR1520_HH" , "B12_p95" , "SAR20_HH"]
